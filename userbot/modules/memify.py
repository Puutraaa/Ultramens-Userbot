...
@bot.on(geezbot_cmd(outgoing=True, pattern=r"mmf(?: |$)(.*)"))
async def mim(event):
    if event.fwd_from:
        return
...
@bot.on(geezbot_cmd(outgoing=True, pattern=r"mmf2(?: |$)(.*)"))
async def mim(event):
    if event.fwd_from:
        return
    if not event.reply_to_msg_id:
...
CMD_HELP.update({
    "memify":
        f"`{geez}mmf` texttop ; textbottom\
        \nUsage: Reply a sticker/image/gif and send with cmd.\n"
        "`{geez}mmf2` texttop ; textbottom\
        \nUsage: Reply a sticker/image/gif and send with cmd."
})
